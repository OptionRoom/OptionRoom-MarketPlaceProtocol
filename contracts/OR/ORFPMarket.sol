...
contract ORFPMarket is FixedProductMarketMaker {
    ...
    function castGovernanceApprovalVote(bool approveFlag) public {
        ...
        uint8 aproveSelection = 0;
        if(approveFlag) { aproveSelection = 1; }
        ...
        if(marketPendingVotersInfo[account].power != 0){
            ...
            marketPendingVotersInfo[account].selection = aproveSelection;
            marketPendingVotersInfo[account].power = ORGovernance.getPowerCount(msg.sender);
        }

        marketPendingVotesCount[aproveSelection] += marketPendingVotersInfo[account].power;
    }
    ...
    function disputeMarket(string memory disputeReason) public{ 
        ...
        require(marketDisputersInfo[account].tottalBalances == 0, "User already dispute");
        ...
        marketDisputersInfo[account].tottalBalances = userTotalBalances;
